...
//預設參數允許為函數參數設定預設值，當調用函數時沒有提供該參數或傳入undefined時，就會使用預設值
...
function getDate() {
    return Date.now(); // 該函式回傳現在時間
}


console.log('===剩餘參數===');
//剩餘參數使用...語法，可以將不定數量的參數收集到一個陣列中
// ! 剩餘參數必須是最後一個參數
...
// 解構參數允許你直接從傳入的物件或陣列中提取值作為參數
function location([x, y, z = 0]) { //預設為 0 (若不預設，未定義值為 undefined)
    return `座標：(${x}, ${y}, ${z})`;
}

console.log(location([10, 20]));
console.log(location([5, 15, 25]));
